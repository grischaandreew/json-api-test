...
trait Translatable
{
    ...
    {
        return $this->hasMany($this->getTranslationModelName(), $this->getRelationKey());//->select($this->translatedAttributes);
    }
    ...
    private function getAttributeOrFallback($locale, $attribute)
    {
        ...
        if( $value !== null) $value = $value->$attribute;
        $usePropertyFallback = $this->useFallback() && $this->usePropertyFallback();
        if (empty($value) && $usePropertyFallback) {
            $trans = $this->getTranslation($this->getFallbackLocale(), true);
            return $trans ? $trans->$attribute : null; 
        }
